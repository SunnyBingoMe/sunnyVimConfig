...
" LastModified:	2011.08.07
" Update: me@SunnyBoy.Me , Date: 2012.05.03
" License:		GPL
...
let s:TBrowser_Config .= '--regex-txt="/^\s*([0-9]+\.?[ \t]+)(.+$)/\1\2/c,content/" '
if (TxtBrowser_Title_Level >= 2)
	let s:TBrowser_Config .= '--regex-txt="/^\s*(([0-9]+\.){1}([0-9]+\.?)[ \t]+)(.+$)/.   \1\4/c,content/" '
endif
if (TxtBrowser_Title_Level >= 3)
	let s:TBrowser_Config .= '--regex-txt="/^\s*(([0-9]+\.){2}([0-9]+\.?)[ \t]+)(.+$)/.       \1\4/c,content/" '
endif
if (TxtBrowser_Title_Level >= 4)
	let s:TBrowser_Config .= '--regex-txt="/^\s*(([0-9]+\.){3}([0-9]+\.?)[ \t]+)(.+$)/.           \1\4/c,content/" '
endif
